...
// export default async function MedicineCard() {
...
export default function MedicineCard({ id, name, genericName, price, imageUrl }: MedicineType) {
    const safeUrl = imageUrl ? encodeURI(imageUrl) : null;

    return (
        <article className="w-full bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            <Link href={`/shop/${id}`} className="block">
                <div className="relative w-full h-36 bg-gray-100 rounded-md overflow-hidden mb-2">
                    {safeUrl ? (
                        <Image src={safeUrl} alt={name || "Medicine Image"} fill className="object-cover" sizes="(max-width:768px)100vw,33vw" />
                    ) : (
                        <div className="w-full h-full flex items-center justify-center text-gray-400">No image</div>
                    )}
                </div>
